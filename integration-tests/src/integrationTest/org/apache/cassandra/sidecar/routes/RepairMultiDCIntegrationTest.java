 ...
 */
@Tag("heavy")
public class RepairMultiDCIntegrationTest extends SharedClusterSidecarIntegrationTestBase
{
     ...
     */
    @Test
    void testCrossDCRepair(VertxTestContext context)
    {
        ...
        validateDataConsistency(table, List.of(1, 2, 3, 4), NUM_ROWS);
     ...
     */
    @Test
    void testSingleDCRepair(VertxTestContext context)
    {
        QualifiedName table = createUniqueTestTable(TEST_TABLE_PREFIX, CREATE_STMT);
        ...
        // Create data inconsistency within datacenter1 (write to node 1, not node 3)
        populateDataSingleNode(table, 1, NUM_ROWS); // Node 1 is in datacenter1
        
        // Verify initial state - only node 1 has data
        validateDataConsistency(table, List.of(1), NUM_ROWS);      // datacenter1 node 1 has data
        validateDataConsistency(table, List.of(2, 3, 4), 0);      // All other nodes have no data
        ...
        validateDataConsistency(table, List.of(1, 3), NUM_ROWS);   // datacenter1 nodes should be consistent
        validateDataConsistency(table, List.of(2, 4), 0);         // datacenter2 nodes should still have no data
     ...
     */
    @Test
    void testCrossDCIncrementalRepair(VertxTestContext context)
    {
        QualifiedName table = createUniqueTestTable(TEST_TABLE_PREFIX, CREATE_STMT);
        ...
        populateDataSingleNode(table, 1, NUM_ROWS);
        validateDataConsistency(table, List.of(1), NUM_ROWS);
        validateDataConsistency(table, List.of(2, 3, 4), 0);
