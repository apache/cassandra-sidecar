...
public class SpiffeCertificateValidator implements MutualTlsCertificateValidator
{
    private static String getSANSpiffe(final Certificate[] clientCertificates) throws CertificateException
    {
        ...
        if (subjectAltNames != null)
        {
            for (List<?> item : subjectAltNames)
            {
                ...
                if (type == uriType && spiffe.startsWith("spiffe://"))
                {  // Spiffe is a URI
                    return spiffe;
                }
            }
    ...
    public static X509Certificate[] castCertsToX509(Certificate[] clientCertificateChain)
    {
        ...
        return Arrays.stream(clientCertificateChain).filter(certificate -> certificate instanceof X509Certificate).toArray(X509Certificate[]::new);
    }
    ...
    public boolean isValidCertificate(Credentials credentials)
    {
        ...
        MutualTlsCredentials creds;
        try
        {
            creds = (MutualTlsCredentials) credentials;
        }
