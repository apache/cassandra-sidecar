                      ...
                      UpgradeableCluster cluster)
    {
        ...
        assertThat(localTokenRanges).hasSize(1)
                                    .containsEntry(MANAGED_CASSANDRA_NODE_NUM,
                                            ImmutableSet.of(new TokenRange(Long.MIN_VALUE, 0), new TokenRange(0, 1000), new TokenRange(1500, Long.MAX_VALUE)));
        ...
        startAsync("Start new node node" + joiningNode.config()
                                                      .num(),
        ...
        assertThat(localTokenRanges)
                                    ...
                                    .hasSize(1)
                                    .containsEntry(MANAGED_CASSANDRA_NODE_NUM, ImmutableSet.of(new TokenRange(Long.MIN_VALUE, 0), new TokenRange(0, 1000),
                                            // (1500, 2000] remains temporarily while new node is joining; once node joined, the range is no longer owned
                                            new TokenRange(1500, 2000), new TokenRange(2000, Long.MAX_VALUE)));
        ...
        loopAssert(10, 500, () -> {
            ...
            assertThat(restoreRanges).describedAs("Local token ranges are effectively the same. Therefore restore ranges do not change")
                                     .hasSize(1);
            assertRestoreRange(ranges.get(0), 1500L, 1600L);
        });
    }
                                                   ...
                                                   int joiningNodeNum)
    {
        ...
        return cassandraTestContext.configureAndStartCluster(builder -> {
            builder.withInstanceInitializer((cl,
                                             num) -> BBHelperJoiningNode.install(cl, num, joiningNodeNum));
