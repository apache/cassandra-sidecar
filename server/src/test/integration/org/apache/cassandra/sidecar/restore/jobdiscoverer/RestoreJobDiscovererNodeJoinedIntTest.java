...
class RestoreJobDiscovererNodeJoinedIntTest extends IntegrationTestBase
{
                      ...
                      UpgradeableCluster cluster)
    {
        ...
        // create slice
        short bucketId = 0;
        CreateSliceRequestPayload slicePayload = new CreateSliceRequestPayload("sliceId", bucketId, "bucket", "key", "checksum", BigInteger.valueOf(1L),
        ...
        assertThat(localTokenRanges).hasSize(1)
                                    .containsEntry(NODE, ImmutableSet.of(new TokenRange(0, 1000), new TokenRange(1000, 1500)))
        ...
        assertThat(ranges.get(0)
                         .tokenRange()).isEqualTo(new TokenRange(0, 1000));
        assertThat(ranges.get(1)
                         .tokenRange()).isEqualTo(new TokenRange(1000, 1500));

        // Decommission
        IUpgradeableInstance seed = cluster.get(1);
        IUpgradeableInstance joiningNode = addInstance(cluster, seed.config()
                                                                    .localDatacenter(),
                seed.config()
                    .localRack(),
                inst -> {
        ...
        loopAssert(60, 1000, () -> {
            ...
            assertThat(currentLocalTokenRanges).hasSize(2)
                                               .containsEntry(NODE, ImmutableSet.of(new TokenRange(0, 1000), new TokenRange(1000, 1500)))
                                               .containsEntry(NODE_JOINED, ImmutableSet.of(new TokenRange(1000, 1500), new TokenRange(1500, 2000)));
            List<RestoreRange> restoreRanges = rangeDatabaseAccessor.findAll(jobId, bucketId);
            assertThat(restoreRanges).hasSize(3);
            assertThat(restoreRanges.get(0)
                                    .tokenRange()).isEqualTo(new TokenRange(0, 1000));
            assertThat(restoreRanges.get(1)
                                    .tokenRange()).isEqualTo(new TokenRange(1000, 1500));
            assertThat(restoreRanges.get(2)
                                    .tokenRange()).isEqualTo(new TokenRange(1500, 1600));
        });
    }
