...
public class SidecarLoadBalancingPolicyTest extends IntegrationTestBase
{
    ...
    {
        Set<Host> hosts = sidecarTestContext.session()
                                            .getCluster()
                                            .getMetadata()
                                            .getAllHosts();
        ...
        IUpgradeableInstance inst = shutDownNonLocalInstance(cluster, sidecarTestContext.instancesMetadata()
                                                                                        .instances());
        assertThat(inst.isShutdown()).isTrue();
        InetSocketAddress downInstanceAddress = new InetSocketAddress(inst.broadcastAddress()
                                                                          .getAddress(),
                inst.config()
    ...
    private void assertConnectionsWithRetry(InetSocketAddress downInstanceAddress,
                                            int expectedConnections)
    {
        ...
        String message = String.format("Waited 2 minutes for connected hosts (%d) to be the expected number (%d) but failed", connectedHosts.size(),
                expectedConnections);
    ...
    {
        Set<InetSocketAddress> localInstances = instances.stream()
                                                         .map(i -> new InetSocketAddress(i.host(), i.port()))
                                                         .collect(Collectors.toSet());
        for (IUpgradeableInstance inst : cluster)
        {
            InetSocketAddress nativeAddress = new InetSocketAddress(inst.config()
                                                                        .broadcastAddress()
                                                                        .getAddress(),
                    inst.config()
                        .getInt("native_transport_port"));
