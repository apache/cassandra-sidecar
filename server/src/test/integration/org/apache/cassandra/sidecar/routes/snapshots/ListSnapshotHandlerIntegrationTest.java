...
class ListSnapshotHandlerIntegrationTest extends IntegrationTestBase
{
    ...
    void testListSnapshot(VertxTestContext context) throws Exception
    {
        ...
        createSnapshot(client, testRoute).compose(response -> {
            assertThat(response.statusCode()).isEqualTo(OK.code());

            // validate that the snapshot is created
            List<Path> found = findChildFile(sidecarTestContext, "127.0.0.1", TEST_KEYSPACE, "rank_snapshot");
            ...
            return listSnapshot(client, testRoute);
                                         ...
                                         .compose(response -> {
                                             ...
                                             assertThat(listResponse.snapshotFilesInfo()
                                                                    .stream()).noneMatch(f -> f.fileName.contains(".ryear"));
                                         ...
                                         .onSuccess(response -> {
                                             ...
                                             assertThat(listResponse.snapshotFilesInfo()
                                                                    .stream()).anyMatch(f -> f.fileName.startsWith(".ryear"));
                               ...
                               String tableName)
    {
        ...
        {
            session.execute("INSERT INTO " + TEST_KEYSPACE + "." + tableName + " (race_year, race_name, cyclist_name, rank) " + "VALUES (" + (1990 + i)
                    + ", 'Tour_" + i + "', 'Benjamin_" + i + "', " + (i + 1) + ");");
        }
    }

    private Future<HttpResponse<String>> createSnapshot(WebClient client,
                                                        String testRoute)
    {
    ...
    private Future<HttpResponse<Buffer>> listSnapshot(WebClient client,
                                                      String testRoute)
    {
