...
class ClearSnapshotHandlerIntegrationTest extends IntegrationTestBase
{
    ...
    @CassandraIntegrationTest
    void deleteSnapshotFailsWhenTableDoesNotExist(VertxTestContext context) throws Exception
    {
        createTestKeyspace();
    ...
    void testDeleteSnapshotEndpoint(VertxTestContext context) throws Exception
    {
        createTestKeyspace();
        ...
        String testRoute = String.format("/api/v1/keyspaces/%s/tables/%s/snapshots/%s", tableName.maybeQuotedKeyspace(), tableName.maybeQuotedTableName(),
                snapshotName);
        ...
        client.put(server.actualPort(), "127.0.0.1", testRoute)
              ...
              .send(context.succeeding(createResponse -> context.verify(() -> {
                  ...
                  final List<Path> found = findChildFile(sidecarTestContext, "127.0.0.1", tableName.keyspace(), snapshotName);
                  ...
                  client.delete(server.actualPort(), "127.0.0.1", testRoute)
                        ...
                        .send(context.succeeding(deleteResponse -> context.verify(() -> {
                            ...
                            final List<Path> found2 = findChildFile(sidecarTestContext, "127.0.0.1", tableName.keyspace(), snapshotName);
