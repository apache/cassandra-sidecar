...
public class SidecarSchemaTest
{
    ...
    void testSchemaInitOnStartup()
    {
        ...
        loopAssert(10, 500, () -> {
            ...
            "INSERT INTO sidecar_internal.cluster_ops (  cluster_name,  operation_id,  operation_type,  status,  node_execution_order,  operation_metadata) VALUES (?, ?, ?, ?, ?, ?)",
            "SELECT cluster_name, operation_id, operation_type, status, node_execution_order, operation_metadata FROM sidecar_internal.cluster_ops WHERE cluster_name = ? AND operation_id = ?",
            "UPDATE sidecar_internal.cluster_ops SET status = ? WHERE cluster_name = ? AND operation_id = ? AND operation_type = ?",
            "SELECT cluster_name, operation_id, operation_type, status, node_execution_order, operation_metadata FROM sidecar_internal.cluster_ops WHERE cluster_name = ? LIMIT ?",

            "INSERT INTO sidecar_internal.active_cluster_ops (cluster_name, operation_type, operation_id) VALUES (?, ?, ?) IF NOT EXISTS",
            "SELECT operation_type, operation_id FROM sidecar_internal.active_cluster_ops WHERE cluster_name = ?",
            "SELECT operation_id FROM sidecar_internal.active_cluster_ops WHERE cluster_name = ? AND operation_type = ?",
            "DELETE FROM sidecar_internal.active_cluster_ops WHERE cluster_name = ? AND operation_type = ? IF operation_id = ?",
