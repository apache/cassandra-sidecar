...
class FeaturePermissionTest
{
    ...
    {
        Authorization bulkReadAuthorization = BULK_READ_DIRECT.permission().toAuthorization("keyspace=university,table=student");

        assertThat(bulkReadAuthorization.verify(READ_RING.toAuthorization("data/university"))).isTrue();
        assertThat(bulkReadAuthorization.verify(READ_SCHEMA.toAuthorization("data/university"))).isTrue();
        ...
        Authorization bulkWriteAuthorization = BULK_WRITE_DIRECT.permission().toAuthorization("keyspace=university,table=student");
    ...
    {
        Authorization bulkReadAuthorization = BULK_READ_DIRECT.permission().toAuthorization("keyspace=university,table=*");
        ...
        Authorization bulkWriteAuthorization = BULK_WRITE_DIRECT.permission().toAuthorization("keyspace=university,table=*");
    ...
    {
        Permission repairPermission = new StandardPermission("REPAIR");

        CompositePermission compositePermission
        = new CompositePermission("featureX", Collections.singletonList(Pair.of(repairPermission, "{dc}")));

        Authorization compositeAuthorization = compositePermission.toAuthorization("dc=DC1");
