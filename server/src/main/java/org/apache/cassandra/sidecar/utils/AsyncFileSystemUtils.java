...
public class AsyncFileSystemUtils
{
     ...
     */
    public static Future<FileStoreProps> fileStoreProps(String path,
    ...
    /**
     * Asynchronously check whether the file store can satisfy the required unusable space percentage. The check fails when the current usable space falls below
     * the required usable space percentage.
     *
     * @param path path to locate its belonging {@link FileStore}
    ...
    /**
     * Asynchronously check whether the file store can satisfy both the requested space and the required unusable space percentage. If any of the condition
    ...
    public static Future<Void> ensureSufficientStorage(String path,
                                                       ...
                                                       TaskExecutorPool executorPool)
    {
        return fileStoreProps(path, executorPool).map(props -> { // A confidence check. It should not happen.
            if (props.usableSpace > props.totalSpace)
            {
                LOGGER.error("Invalid file store state. fileStore={} path={} available={} total={}", props.name, path, props.usableSpace, props.totalSpace);
                throw new IllegalStateException("FileStore has less total space than usable space. " + "FileStore: " + props.name + ", total space: "
                        + props.totalSpace + ", usable space: " + props.usableSpace);
            }
            return props;
        })
                                                 .compose(props -> {
                                                     ...
                                                     if (deductedUsableSpace < requiredUsableSpace)
                                                     {
                                                         ...
                                                         return Future.failedFuture(new InsufficientStorageException(props.name, path, props.totalSpace,
                                                                 props.usableSpace, requiredUsableSpace));
                                                     }
    ...
    public static class FileStoreProps
    {
        ...
        public FileStoreProps(String name,
                              long totalSpace,
                              long usableSpace,
