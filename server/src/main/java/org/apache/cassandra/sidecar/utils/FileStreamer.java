...
public class FileStreamer
{
    ...
    /**
     * Streams the {@code filename file} with length {@code fileLength} for the (optionally) requested {@code rangeHeader} using the provided {@code response}.
     *
     ...
     */
    public Future<Void> stream(HttpResponse response,
                               int instanceId,
                               String filename,
                               long fileLength,
                               String rangeHeader)
    {
        return parseRangeHeader(rangeHeader, fileLength).compose(range -> stream(response, instanceId, filename, fileLength, range));
    }

    /**
     * Streams the {@code filename file} with length {@code fileLength} for the requested {@code range} using the provided {@code response}.
     *
     * @param response the response to use
     * @param instanceId Cassandra instance from which file is streamed
     * @param filename the path to the file to serve
    ...
    /**
     * Send the file if rate-limiting is disabled or when it successfully acquires a permit from the {@link SidecarRateLimiter}.
     *
     * @param response the response to use
     * @param instanceId Cassandra instance from which file is streamed
     * @param filename the path to the file to serve
    ...
    {
        InstanceMetrics instanceMetrics = instanceMetadataFetcher.instance(instanceId)
                                                                 .metrics();
        ...
        if (acquire(response, instanceId, filename, fileLength, range, startTimeNanos, streamSSTableMetrics, promise))
        {
            ...
            LOGGER.debug("Streaming range {} for file {} to client {}. Instance: {}", range, filename, response.remoteAddress(), response.host());
            response.sendFile(filename, fileLength, range)
                    .onSuccess(v -> {
                        String component = parseSSTableComponent(filename);
                        LOGGER.debug("Streamed file {} successfully to client {}. Instance: {}", filename, response.remoteAddress(), response.host());
    ...
    /**
     * Acquires a permit from the {@link SidecarRateLimiter} if it can be acquired immediately without delay. Otherwise, it will retry acquiring the permit
     * later in the future until it exhausts the retry timeout, in which case it will ask the client to retry later in the future.
     *
     * @param response the response to use
     * @param instanceId Cassandra instance from which file is streamed
     * @param filename the path to the file to serve
     ...
     */
    private boolean acquire(HttpResponse response,
                            int instanceId,
                            String filename,
                            long fileLength,
                            HttpRange range,
                            ...
                            Promise<Void> promise)
    {
        ...
        {
            LOGGER.warn("Retries for acquiring permit exhausted for client {}. Instance: {}. " + "Asking client to retry after {} nanoseconds.",
                    response.remoteAddress(), response.host(), waitTimeNanos);
        ...
        {
            LOGGER.debug("Retrying streaming after {} nanos for client {}. Instance: {}", waitTimeNanos, response.remoteAddress(), response.host());
            executorPools.service()
                         ...
                         .setTimer(NANOSECONDS.toMillis(waitTimeNanos) + 1,
                                 t -> acquireAndSend(response, instanceId, filename, fileLength, range, startTimeNanos, promise));
        }
        return false;
    }
     ...
     *
     * @return true if we exceeded timeout, false otherwise Note: for this check we take wait time for the request into consideration
     */
    private boolean isTimeoutExceeded(long startTimeNanos,
                                      long waitTimeNanos)
    {
        long nowNanos = System.nanoTime();
        long timeoutNanos = startTimeNanos + waitTimeNanos + config.timeout()
                                                                   .to(NANOSECONDS);
     ...
     */
    public Future<HttpRange> parseRangeHeader(String rangeHeader,
                                              long fileLength)
    {
