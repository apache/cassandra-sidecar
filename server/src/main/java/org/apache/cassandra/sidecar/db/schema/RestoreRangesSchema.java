...
/**
 * {@link RestoreRangesSchema} holds all prepared statements needed for talking to Cassandra for various actions related to restore progress tracking in terms
 * of each token range
 */
public class RestoreRangesSchema extends TableSchema implements ExecuteOnClusterLeaseholderOnly
{
    ...
    {
        return String.format("CREATE TABLE IF NOT EXISTS %s.%s (" + "  job_id timeuuid," + "  bucket_id smallint," + // same bucket_id as in the slice row
                "  start_token varint," + // exclusive
                "  end_token varint," + // inclusive
                "  slice_id text," + "  slice_bucket text," + "  slice_key text," + "  status_by_replica map<text, text>," + // key is the replica; value is the
                                                                                                                             // status
                "  PRIMARY KEY ((job_id, bucket_id), start_token, end_token)" + ") WITH default_time_to_live = %s", keyspaceConfig.keyspace(), TABLE_NAME,
                tableTtl.toSeconds());
    }
    ...
    private static class CqlLiterals
    {
        static String createRange(SchemaKeyspaceConfiguration config)
        {
            ...
            return withTable("UPDATE %s.%s " + "SET slice_id = ?, slice_bucket = ?, slice_key = ?, status_by_replica = status_by_replica + ? "
        ...
        {
            return withTable("SELECT job_id, bucket_id, slice_id, slice_bucket, slice_key, " + "start_token, end_token, status_by_replica " + "FROM %s.%s "
                    + "WHERE job_id = ? AND bucket_id = ? ALLOW FILTERING", config);
        }

        static String update(SchemaKeyspaceConfiguration config)
        {
            return withTable("UPDATE %s.%s " + "SET status_by_replica = status_by_replica + ? "
                    + "WHERE job_id = ? AND bucket_id = ? AND start_token = ? AND end_token = ?", config);
        }
