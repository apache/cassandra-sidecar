...
public abstract class TaskExecutorPool implements WorkerExecutor
{
    ...
    private TaskExecutorPool(Vertx vertx,
                             WorkerPoolConfiguration config)
    {
        this.vertx = vertx;
        this.workerExecutor = vertx.createSharedWorkerExecutor(config.workerPoolName(), config.workerPoolSize(), config.workerMaxExecutionTime()
                                                                                                                       .quantity(),
                config.workerMaxExecutionTime()
                      .unit());
    }

    /**
     * Like {@link #setPeriodic(long, Handler, boolean)} with the handler guaranteed to be executed in a worker thread and ordered = false.
     *
     ...
     * @param delay the delay in milliseconds, after which the timer will fire
     * @param handler the handler that will be called with the timer ID when the timer fires
     * @param ordered if true and if executeBlocking is called several times on the same context, the executions for that context will be executed serially, not
     *            in parallel. The periodic scheduled executions could be delayed if the prior execution on the same context is taking longer. If false then
     *            they will be no ordering guarantees
     * @return the unique identifier for the timer
     */
    public long setPeriodic(long delay,
                            Handler<Long> handler,
    ...
    /**
     * Set a periodic timer to fire every {@code delay} milliseconds with initial delay, at which point {@code handler} will be called with the id of the timer.
     *
     ...
     * @param delay the delay in milliseconds, after which the timer will fire
     * @param handler the handler that will be called with the timer ID when the timer fires
     * @param ordered if true then executeBlocking is called several times on the same context, the executions for that context will be executed serially, not
     *            in parallel. if false then they will be no ordering guarantees
     * @return the unique ID of the timer
     */
    public long setPeriodic(long initialDelay,
                            long delay,
                            ...
                            boolean ordered)
    {
        return vertx.setPeriodic(initialDelay, delay, id -> workerExecutor.executeBlocking(() -> {
            handler.handle(id);
            return id;
        }, ordered));
    }
    ...
    /**
     * Like {@link #setTimer(long, Handler)} with the handler guaranteed to be executed in a worker thread and ordered = false.
     *
     * @param delay the delay in milliseconds, after which the timer will fire
     * @param handler the handler that will be called with the timer ID when the timer fires
     * @return the unique identifier for the timer
    ...
    /**
     * A helper that converts the {@code Runnable} to a {@code Callable} and calls {@link #executeBlocking(Callable, boolean)}
     */
    public Future<Void> runBlocking(ThrowingRunnable runnable,
                                    boolean ordered)
    {
        ...
        return executeBlocking(callable, ordered);
    }
    ...
    /**
     * {@link ServiceTaskExecutorPool} is used for executing tasks that are short lived and not expected to block for too long, therefore will free up resources
     * more quickly
     */
    static class ServiceTaskExecutorPool extends TaskExecutorPool
    {
        ...
        ServiceTaskExecutorPool(Vertx vertx,
                                WorkerPoolConfiguration config,
        ...
        protected void recordTimeTaken(long durationNanos)
        {
            ...
            metrics.server()
                   .resource().serviceTaskTime.metric.update(durationNanos, TimeUnit.NANOSECONDS);
        }
    }

    /**
     * {@link InternalTaskExecutorPool} is used for executing tasks that are expected to block for a long time, and the nature of these tasks is that they are
     * not expected to complete immediately and can be queued if all the threads are busy
     */
    static class InternalTaskExecutorPool extends TaskExecutorPool
    {
        private final SidecarMetrics metrics;

        InternalTaskExecutorPool(Vertx vertx,
                                 WorkerPoolConfiguration config,
                                 SidecarMetrics metrics)
        {
            super(vertx, config);
        ...
        protected void recordTimeTaken(long durationNanos)
        {
            ...
            metrics.server()
                   .resource().internalTaskTime.metric.update(durationNanos, TimeUnit.NANOSECONDS);
        }
    }
}
