...
public class CassandraInputValidator
{
    ...
    /**
     * Validates that the {@code keyspace} is not {@code null}, that it contains valid characters, and that it's not a forbidden keyspace.
     *
     ...
     * @throws HttpException when the {@code keyspace} contains invalid characters in the name or when the keyspace is forbidden
     */
    public Name validateKeyspaceName(@NotNull String keyspace)
    {
        ...
        if (validationConfiguration.forbiddenKeyspaces()
                                   .contains(name.name()))
    ...
    /**
     * Validates that the {@code tableName} is not {@code null}, and it contains allowed character for Cassandra table names.
     *
    ...
    /**
     * Validates that the {@code snapshotName} is not {@code null}, and it contains allowed character for the Cassandra snapshot names.
     *
    ...
    /**
     * Validates that the {@code componentName} is not {@code null}, and it contains allowed names for the Cassandra SSTable component.
     *
    ...
    {
        return validateComponentNameByRegex(componentName, validationConfiguration.allowedPatternForComponentName());
    }

    /**
     * Validates that the {@code componentName} is not {@code null}, and it contains a subset of allowed names for the Cassandra SSTable component.
     *
     * @param componentName the name of the SSTable component to validate
     * @return the validated {@code componentName}
     * @throws NullPointerException when the {@code componentName} is null
     * @throws HttpException when the {@code componentName} is not a valid name for the configured restricted component name
     */
    public String validateRestrictedComponentName(@NotNull String componentName)
    {
        return validateComponentNameByRegex(componentName, validationConfiguration.allowedPatternForRestrictedComponentName());
    }
     ...
     */
    @NotNull
    private String validateComponentNameByRegex(String componentName,
                                                String regex)
    {
     ...
     */
    public void validatePattern(String unquotedInput,
                                String maybeQuoted,
                                String exceptionHint,
                                boolean isQuotedFromSource)
    {
        String pattern = isQuotedFromSource ? validationConfiguration.allowedPatternForQuotedName() : validationConfiguration.allowedPatternForName();
    ...
    /**
     * Removes the surrounding quotes for the name, if the quotes are present. Otherwise, returns the original input. Validates that the {@code name} matches
     * the {@code patternWordChars}
     *
     * @param name name
     * @param exceptionHint hint to add in the exception message
     * @throws HttpException when the {@code unquotedInput} does not match the pattern
     */
    public void validateNamePattern(Name name,
                                    String exceptionHint)
    {
