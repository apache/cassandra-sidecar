...
public abstract class AbstractHandler<T> implements Handler<RoutingContext>
{
     ...
     */
    protected void handleOperationalJob(
    OperationalJobManager jobManager, ServiceConfiguration config, RoutingContext context, OperationalJob job, boolean checkConflict)
    {
        try
        {
            jobManager.trySubmitJob(job, checkConflict);
        }
