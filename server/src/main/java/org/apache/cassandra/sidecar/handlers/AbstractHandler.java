...
/**
 * An abstract {@link Handler Handler&lt;RoutingContext&gt;} that provides common functionality for handler implementations.
 *
...
public abstract class AbstractHandler<T> implements Handler<RoutingContext>
{
     ...
     */
    protected AbstractHandler(InstanceMetadataFetcher metadataFetcher,
    ...
    public void handle(RoutingContext context)
    {
        ...
        try
        {
            ...
            logger.debug("{} received request={}, remoteAddress={}, instance={}", this.getClass()
                                                                                      .getSimpleName(),
    ...
    /**
     * Returns the host from the path if the requests contains the {@code /instance/} path parameter, otherwise it returns the host parsed from the request.
     *
    ...
    public String host(RoutingContext context)
    {
        ...
        {
            String instanceIdParam = context.request()
            ...
            try
            {
                ...
                return metadataFetcher.instance(instanceId)
                                      .host();
            }
                                  ...
                                  T request)
    {
        ...
        {
            logger.warn("{} request failed due to client error. request={}, remoteAddress={}, instance={}", this.getClass()
                                                                                                                .getSimpleName(),
                    request, remoteAddress, host, cause);
        }
        else if (HttpStatusClass.SERVER_ERROR.contains(httpException.getStatusCode()))
        {
            logger.error("{} request failed due to server error. request={}, remoteAddress={}, instance={}", this.getClass()
                                                                                                                 .getSimpleName(),
                    request, remoteAddress, host, cause);
        }
    ...
    /**
     * Returns the validated {@link QualifiedTableName} from the context, where the both keyspace and table name are required.
     *
    ...
    protected QualifiedTableName qualifiedTableName(RoutingContext context,
                                                    boolean required)
    {
        return new QualifiedTableName(keyspace(context, required), tableName(context, required));
    }
     ...
     */
    protected Name keyspace(RoutingContext context,
                            boolean required)
    {
     ...
     */
    private Name tableName(RoutingContext context,
                           boolean required)
    {
    ...
    /**
     * Given a combined host address like 127.0.0.1:9042 or [2001:db8:0:0:0:ff00:42:8329]:9042, this method removes port information and returns 127.0.0.1 or
