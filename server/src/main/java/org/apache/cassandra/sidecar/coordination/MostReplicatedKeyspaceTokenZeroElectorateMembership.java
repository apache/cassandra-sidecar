 ...
 * instances it manages owns token {@code 0} for the user keyspace that has the highest replication factor. If multiple keyspaces have the highest replication
 * factor, the keyspace to be used is decided by the keyspace with the name that sorts first in the lexicographic sort order. If no user keyspaces are created,
 * the internal sidecar keyspace will be used.
 */
public class MostReplicatedKeyspaceTokenZeroElectorateMembership extends AbstractTokenZeroOfKeyspaceElectorateMembership
{
    ...
    /**
     * Performs pre-checks ensuring local instances are configured; an active session to the database is present; and returns the keyspace with the highest
     * replication factor. If multiple keyspaces have the highest replication factor, the keyspace to be used is decided by the keyspace with the name that
     * sorts first in the lexicographic sort order. Defaults to the sidecar keyspace when there are no user keyspaces.
     *
     * @return user keyspace
     */
    String highestReplicationFactorKeyspace()
    {
        if (instanceMetadataFetcher.allLocalInstances()
                                   .isEmpty())
        {
        ...
        Set<String> forbiddenKeyspaces = configuration.cassandraInputValidationConfiguration()
                                                      .forbiddenKeyspaces();
        String sidecarKeyspaceName = configuration.serviceConfiguration()
                                                  .schemaKeyspaceConfiguration()
                                                  .keyspace();

        return activeSession.getCluster()
                            .getMetadata()
                            .getKeyspaces()
                            .stream()
    ...
    int aggregateReplicationFactor(KeyspaceMetadata keyspace)
    {
        ...
        for (String value : keyspace.getReplication()
                                    .values())
        {
            try
            {
